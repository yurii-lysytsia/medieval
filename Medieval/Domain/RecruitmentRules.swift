...
public enum RecruitmentRules {
    ...
    public static func recruit(
        ...
        guard let level = cityLevels.first(where: { $0.id == city.levelID }) else { return .failure(.recruitmentLimitReached(cityID, limit: 0)) }
        ...
        let sequence = world.units.filter { $0.ownerID == playerID }.count + 1
